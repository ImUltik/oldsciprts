...
local function playNext()
	if #musicQueue == 0 then 
		...
		musicPanel.Menu.TogglePlaying.ImageRectOffset = Vector2.new(764, 244)
		return 
	end
	...
	local soundFile = musicQueue[1].sound
	local fullPath = siriusValues.siriusFolder.."/Music/"..soundFile
	
	-- validate asset resolution
	local okAsset, asset = pcall(getcustomasset, fullPath)
	if not okAsset or not asset or asset == "" then
		queueNotification("Playback Error", "Failed to load: "..soundFile.." (asset resolve failed)", 4370341699)
		
		-- Remove from queue list UI
	...
	if checkSetting("Now Playing Notifications").current then 
		queueNotification("Now Playing", soundFile, 4400695581) 
	end
	...
	musicPanel.Playing.Text = soundFile
	...
	if not okPlay then
		queueNotification("Playback Error", "Failed to play: "..soundFile.." ("..tostring(playErr)..")", 4370341699)
		table.remove(musicQueue, 1)
		playNext()
		return
	end
	
	musicPanel.Menu.TogglePlaying.ImageRectOffset = Vector2.new(804, 124)
...
local function addToQueue(file)
	if not getcustomasset then 
		queueNotification("Unable to queue file", "getcustomasset not available", 4370341699)
		return 
	end
	
	if not file or file == "" then
		queueNotification("Invalid Input", "Please enter a filename (e.g. song.mp3)", 4370341699)
		return
	end
	
	-- Check if file is already in queue
	for _, item in ipairs(musicQueue) do
		if item.sound == file then
			queueNotification("Already in Queue", file.." is already queued", 4370341699)
			return
		end
	end
	...
	local filePath = siriusValues.siriusFolder.."/Music/"..file
	
	-- Try to validate the file can be loaded
	local ok, asset = pcall(getcustomasset, filePath)
	if not ok or not asset or asset == "" then
		queueNotification("File Not Found", "Cannot load: "..file.."\n\nMake sure the file is in:\nSirius/Music/"..file, 4370341699)
		return
	end
	
	queueNotification("Added to Queue", file.." added successfully", 4400695581)
	...
	-- Try to get duration using the already validated asset
	pcall(function()
		local getLength = Instance.new("Sound", workspace)
		getLength.SoundId = asset
		getLength.Volume = 0
		getLength:Play()
		task.wait(0.05)
...
local function getLocalMusicFiles()
	local path = siriusValues.siriusFolder.."/Music"
	local files = {}
	local apiUsed = "none"
	local rawCount = 0
	local debugInfo = ""
	...
	pcall(function()
		-- try common exploit FS listing APIs
		if listfiles then
			apiUsed = "listfiles"
			local success, result = pcall(listfiles, path)
			if success and result then
				rawCount = #result
				debugInfo = "Raw results: "
				for i, f in ipairs(result) do
					if i <= 5 then debugInfo = debugInfo .. tostring(f) .. "; " end
					local name = tostring(f)
					-- extract base filename robustly
					local base = name:match("([^/\\]+)$") or name
					base = base:gsub("^%s+",""):gsub("%s+$","")
					if base ~= "" and base ~= "readme.txt" then table.insert(files, base) end
				end
			end
		elseif listfolder then
			apiUsed = "listfolder"
			local success, result = pcall(listfolder, path)
			if success and result then
				rawCount = #result
				debugInfo = "Raw results: "
				for i, name in ipairs(result) do
					if i <= 5 then debugInfo = debugInfo .. tostring(name) .. "; " end
					local base = tostring(name):match("([^/\\]+)$") or tostring(name)
					base = base:gsub("^%s+",""):gsub("%s+$","")
					if base ~= "" and base ~= "readme.txt" then table.insert(files, base) end
				end
			end
		elseif list_dir then
			apiUsed = "list_dir"
			local success, result = pcall(list_dir, path)
			if success and result then
				rawCount = #result
				debugInfo = "Raw results: "
				for i, name in ipairs(result) do
	...
	local preFilterCount = #files

	-- filter to common audio extensions and unique
	local filtered = {}
	for _, name in ipairs(files) do
		name = tostring(name)
		local lower = string.lower(name)
		if string.find(lower, "%.mp3") or string.find(lower, "%.ogg") or string.find(lower, "%.wav") or string.find(lower, "%.m4a") then
			if not table.find(filtered, name) then 
				table.insert(filtered, name) 
			end
		end
	end

	-- Comprehensive debug notification
	if apiUsed == "none" then
		queueNotification("Music Debug", "No file listing API found (listfiles/listfolder/list_dir not available). Use manual add below.", 4370341699)
	else
		local msg = "API: "..apiUsed.." | Raw: "..rawCount.." | After basename: "..preFilterCount.." | Audio files: "..#filtered
		if #filtered == 0 and rawCount > 0 then
			msg = msg .. "\n" .. debugInfo
		end
		queueNotification("Music Debug", msg, 4370341699)
	end
...
local function refreshMusicLibrary()
	if not getcustomasset then 
		queueNotification("Music Library", "getcustomasset not available in this executor", 4370341699)
		return 
	end
	checkFolder()

	local files = getLocalMusicFiles()
	
	if #files == 0 then 
		-- Don't return - still create the container with instructions
		local container = musicPanel:FindFirstChild("LocalFiles")
		if container then container:Destroy() end
		
		-- Create instruction label
		local instrFrame = Instance.new("Frame")
		instrFrame.Name = "LocalFiles"
		instrFrame.Parent = musicPanel
		instrFrame.BackgroundTransparency = 0.95
		instrFrame.BackgroundColor3 = Color3.fromRGB(15,15,15)
		instrFrame.BorderSizePixel = 0
		instrFrame.Size = UDim2.new(0, 268, 0, 150)
		instrFrame.Position = UDim2.new(0, 6, 0, 50)
		
		local instrLabel = Instance.new("TextLabel")
		instrLabel.Parent = instrFrame
		instrLabel.BackgroundTransparency = 1
		instrLabel.Size = UDim2.new(1, -12, 1, -12)
		instrLabel.Position = UDim2.new(0, 6, 0, 6)
		...
		instrLabel.Text = "No files found.\n\nTo add music manually:\n1. Type the filename (e.g. 'song.mp3') in the Add box below\n2. Click the Play icon\n\nMake sure your MP3/OGG files are in:\nSirius/Music/"
		
		return 
	end

	queueNotification("Music Library", "Found "..tostring(#files).." audio file(s)", 4400695581)

	-- Create a simple scrolling container for local files if it doesn't exist
	local container = musicPanel:FindFirstChild("LocalFiles")
	if not container then
		local sf = Instance.new("ScrollingFrame")
		sf.Name = "LocalFiles"
		sf.Parent = musicPanel
		sf.BackgroundTransparency = 0.95
		sf.BackgroundColor3 = Color3.fromRGB(15,15,15)
		sf.BorderSizePixel = 0
		sf.Size = UDim2.new(0, 268, 0, 150)
		sf.Position = UDim2.new(0, 6, 0, 50)
	...
	for i, file in ipairs(files) do
		...
		label.Text = (string.len(file) > 22) and (string.sub(file,1,20) .. "..") or file
	...
	-- update canvas size
	container.CanvasSize = UDim2.new(0, 0, 0, #files * 32)
end
...
local function openMusic()
	...
	-- Add purple gradient to music panel UIStroke
	if musicPanel:FindFirstChild("UIStroke") and not musicPanel.UIStroke:FindFirstChild("PurpleGradient") then
		local purpleGradient = Instance.new("UIGradient")
		purpleGradient.Name = "PurpleGradient"
		purpleGradient.Color = ColorSequence.new{
			...
			ColorSequenceKeypoint.new(0.4, Color3.fromRGB(147, 112, 219)),
			ColorSequenceKeypoint.new(0.6, Color3.fromRGB(123, 104, 238)),
			ColorSequenceKeypoint.new(0.8, Color3.fromRGB(106, 90, 205)),
			ColorSequenceKeypoint.new(1, Color3.fromRGB(186, 85, 211))
		}
		purpleGradient.Rotation = 0
		purpleGradient.Parent = musicPanel.UIStroke
		
		task.spawn(function()
			while musicPanel and musicPanel.Parent and musicPanel:FindFirstChild("UIStroke") and musicPanel.UIStroke:FindFirstChild("PurpleGradient") do
				tweenService:Create(purpleGradient, TweenInfo.new(3, Enum.EasingStyle.Linear), {Rotation = 360}):Play()
				task.wait(3)
				purpleGradient.Rotation = 0
			end
		end)
	...
	-- Only create file browser if it doesn't exist
	if musicPanel:FindFirstChild("FileBrowser") then
		debounce = false
		return
	end
	...
	browser.Name = "FileBrowser"
	browser.Parent = musicPanel
	browser.BackgroundTransparency = 0.9
	browser.BackgroundColor3 = Color3.fromRGB(20,20,20)
	browser.BorderSizePixel = 0
	browser.Size = UDim2.new(0, 268, 0, 120)
	browser.Position = UDim2.new(0, 6, 0, 50)
	browser.ScrollBarImageColor3 = Color3.fromRGB(100,100,100)
	browser.ScrollBarThickness = 6
	browser.CanvasSize = UDim2.new(0, 0, 0, 0)
	...
	local layout = Instance.new("UIListLayout")
	layout.Parent = browser
	layout.SortOrder = Enum.SortOrder.LayoutOrder
	layout.Padding = UDim.new(0, 3)

	-- Try to list files
	local musicPath = siriusValues.siriusFolder.."/Music"
	local foundFiles = {}
	local apiName = "none"
	local errorMsg = ""

	-- Try each file listing API
	if listfiles then
		apiName = "listfiles"
		local ok, result = pcall(listfiles, musicPath)
		if ok and result then
			for _, fullPath in ipairs(result) do
	...
	elseif listfolder then
		apiName = "listfolder"
		local ok, result = pcall(listfolder, musicPath)
		if ok and result then
			for _, filename in ipairs(result) do
				table.insert(foundFiles, tostring(filename))
			end
		else
			errorMsg = tostring(result)
		end
	else
		errorMsg = "No file listing API available"
	end

	-- Show diagnostic info at top
	local infoLabel = Instance.new("TextLabel")
	infoLabel.Name = "Info"
	infoLabel.Parent = browser
	infoLabel.BackgroundTransparency = 0.5
	infoLabel.BackgroundColor3 = Color3.fromRGB(40,40,80)
	infoLabel.Size = UDim2.new(1, -6, 0, 50)
	infoLabel.Position = UDim2.new(0, 3, 0, 3)
	...
	if apiName == "none" then
		infoLabel.Text = "  No file listing API\n  "..errorMsg.."\n  Use Add box below to type filenames"
		infoLabel.BackgroundColor3 = Color3.fromRGB(80,40,40)
	elseif #foundFiles == 0 then
		infoLabel.Text = "  API: "..apiName.."\n  Files found: 0\n  "..errorMsg
		infoLabel.BackgroundColor3 = Color3.fromRGB(80,60,40)
	else
		infoLabel.Text = "  API: "..apiName.."\n  Files found: "..#foundFiles.."\n  Click to add to queue"
		infoLabel.BackgroundColor3 = Color3.fromRGB(40,80,40)
	end

	-- Create file buttons
	for i, filename in ipairs(foundFiles) do
		local fileBtn = Instance.new("TextButton")
		fileBtn.Name = "File"..i
		fileBtn.Parent = browser
		fileBtn.BackgroundColor3 = Color3.fromRGB(50,50,50)
		fileBtn.BorderSizePixel = 0
		fileBtn.Size = UDim2.new(1, -6, 0, 28)
		...
		fileBtn.MouseEnter:Connect(function()
			fileBtn.BackgroundColor3 = Color3.fromRGB(80,140,80)
		end)
	...
	-- Update canvas size
	browser.CanvasSize = UDim2.new(0, 0, 0, 60 + (#foundFiles * 31))
...
local function openPanel(panelName)
	...
	-- Add purple gradient to panel UIStroke
	if panel:FindFirstChild("UIStroke") and not panel.UIStroke:FindFirstChild("PurpleGradient") then
		local purpleGradient = Instance.new("UIGradient")
		purpleGradient.Name = "PurpleGradient"
		purpleGradient.Color = ColorSequence.new{
			ColorSequenceKeypoint.new(0, Color3.fromRGB(186, 85, 211)),    -- Medium orchid
			ColorSequenceKeypoint.new(0.2, Color3.fromRGB(138, 43, 226)),  -- Blue violet
			ColorSequenceKeypoint.new(0.4, Color3.fromRGB(147, 112, 219)), -- Medium purple
			ColorSequenceKeypoint.new(0.6, Color3.fromRGB(123, 104, 238)), -- Medium slate blue
			ColorSequenceKeypoint.new(0.8, Color3.fromRGB(106, 90, 205)),  -- Slate blue
			ColorSequenceKeypoint.new(1, Color3.fromRGB(186, 85, 211))     -- Medium orchid
		}
		purpleGradient.Rotation = 0
		purpleGradient.Parent = panel.UIStroke
		
		-- Animate the gradient rotation continuously
		task.spawn(function()
			while panel and panel.Parent and panel:FindFirstChild("UIStroke") and panel.UIStroke:FindFirstChild("PurpleGradient") do
				tweenService:Create(purpleGradient, TweenInfo.new(3, Enum.EasingStyle.Linear), {Rotation = 360}):Play()
				task.wait(3)
				purpleGradient.Rotation = 0
			end
...
local function gtaRejoin()
	queueNotification("Rejoining Session", "Teleporting you back to this session...", 4400696294)
	...
	if character and character:FindFirstChild("HumanoidRootPart") then
		...
		-- Get positions - reduced heights
		local startPos = character.HumanoidRootPart.Position + Vector3.new(0, 10, 0)
		local midPos = character.HumanoidRootPart.Position + Vector3.new(0, 150, 0)
		local skyPos = character.HumanoidRootPart.Position + Vector3.new(0, 300, 0)
		
		-- Look down at character initially
		camera.CFrame = CFrame.new(startPos, character.HumanoidRootPart.Position)
		...
		-- First ascent - to middle position
		local camTween1 = tweenService:Create(
			camera,
			TweenInfo.new(1.2, Enum.EasingStyle.Sine, Enum.EasingDirection.Out),
			{CFrame = CFrame.new(midPos, character.HumanoidRootPart.Position)}
		)
		
		local blurTween1 = tweenService:Create(
			blur,
		...
		-- FREEZE 1 - pause in the air
		task.wait(0.7)
		
		-- Second ascent - to sky position
		local camTween2 = tweenService:Create(
			camera,
			TweenInfo.new(1.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
			{CFrame = CFrame.new(skyPos, character.HumanoidRootPart.Position)}
		)
		
		local blurTween2 = tweenService:Create(
			blur,
			TweenInfo.new(1.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
			{Size = 40}
		)
		...
		-- FREEZE 2 - final pause before rejoin
		task.wait(0.5)
		
		-- Store the sky position for when we rejoin
		_G.SiriusRejoinSkyPos = skyPos
		_G.SiriusRejoinCharPos = character.HumanoidRootPart.Position
		...
		-- Fallback to normal rejoin if no character
		rejoin()
	end
end
...
local function serverhop()
	...
	if #servers > 0 then
		...
		-- GTA-style serverhop: instant camera freeze and zoom out with flash
		camera.CameraType = Enum.CameraType.Scriptable
		...
		flashFrame.ZIndex = 10000
		flashFrame.Parent = coreGui
		
		-- Flash in
		tweenService:Create(flashFrame, TweenInfo.new(0.1, Enum.EasingStyle.Linear), {BackgroundTransparency = 0}):Play()
		task.wait(0.1)
		
		-- Instant camera teleport to sky
		local skyPosition = originalCFrame.Position + Vector3.new(0, 500, 0)
		camera.CFrame = CFrame.new(skyPosition, originalCFrame.Position)
		
		-- Flash out
		tweenService:Create(flashFrame, TweenInfo.new(0.3, Enum.EasingStyle.Linear), {BackgroundTransparency = 1}):Play()
		task.wait(0.5)
...
local function ensureFrameProperties()
	...
	-- Create glowing gradient effect for smartBar outline
	if not smartBar.UIStroke:FindFirstChild("UIGradient") then
		...
		gradient.Rotation = 0
		gradient.Parent = smartBar.UIStroke
	end
end
...
local function openHome()
	...
	-- Add purple gradient to home UIStroke
	if homeContainer:FindFirstChild("UIStroke") and not homeContainer.UIStroke:FindFirstChild("PurpleGradient") then
		local purpleGradient = Instance.new("UIGradient")
		purpleGradient.Name = "PurpleGradient"
		purpleGradient.Color = ColorSequence.new{
			ColorSequenceKeypoint.new(0, Color3.fromRGB(186, 85, 211)),
		...
		purpleGradient.Rotation = 0
		purpleGradient.Parent = homeContainer.UIStroke
		
		task.spawn(function()
			while homeContainer and homeContainer.Parent and homeContainer:FindFirstChild("UIStroke") and homeContainer.UIStroke:FindFirstChild("PurpleGradient") do
				tweenService:Create(purpleGradient, TweenInfo.new(3, Enum.EasingStyle.Linear), {Rotation = 360}):Play()
				task.wait(3)
				purpleGradient.Rotation = 0
			end
...
local function openSmartBar()
	...
	tweenService:Create(smartBar.UIStroke, TweenInfo.new(0.8, Enum.EasingStyle.Quint), {Transparency = 0.3}):Play()
...
local function createPlayer(player)
	...
	-- Check if player has admin tag based on user ID
	if siriusValues.adminUsers[player.UserId] then
		local adminTag = siriusValues.adminUsers[player.UserId]
		newPlayer.Role.Text = adminTag
		newPlayer.Role.Visible = true
		newPlayer.Role.TextTransparency = 1
		
		-- Add gradient to owner's name
		if adminTag == "Owner" then
			if not newPlayer.DisplayName:FindFirstChild("UIGradient") then
				local gradient = Instance.new("UIGradient")
				gradient.Color = ColorSequence.new({
				...
				gradient.Rotation = 0
				gradient.Parent = newPlayer.DisplayName
			end
		end
	end
...
musicPanel.AddBox.Input.FocusLost:Connect(function(enterPressed)
	if enterPressed and musicPanel.AddBox.Input.Text ~= "" then
		addToQueue(musicPanel.AddBox.Input.Text)
	end
end)
...
characterPanel.Interactions.Rejoin.Interact.MouseButton1Click:Connect(gtaRejoin)
...
players.PlayerAdded:Connect(function(player)
	...
	if checkSetting("Moderator Detection").current and Pro then
		-- Check if player is in admin user IDs list
		if siriusValues.adminUsers[player.UserId] then
			local adminTag = siriusValues.adminUsers[player.UserId]
			promptModerator(player, adminTag)
			queueNotification("Administrator Joined", adminTag.." ".. player.DisplayName .." has joined your session", 3944670656)
		end
	end
...
runService.RenderStepped:Connect(function(frame)
	...
	-- Animate smartBar gradient outline
	if smartBar.UIStroke:FindFirstChild("UIGradient") then
		local gradient = smartBar.UIStroke.UIGradient
		gradient.Rotation = (gradient.Rotation + (frame * 60)) % 360
	end
end)
...
-- GTA-style zoom in animation when script executes
task.spawn(function()
	local character = localPlayer.Character
	...
	-- Check if we're rejoining (coming from gtaRejoin)
	local startHeight, midHeight, finalHeight, startBlur
	if _G.SiriusRejoinSkyPos and _G.SiriusRejoinCharPos then
		-- Rejoining - start from sky
		startHeight = 300
		midHeight = 150
		finalHeight = 10
		startBlur = 40
		...
		camera.CameraType = Enum.CameraType.Scriptable
		camera.CFrame = CFrame.new(hrp.Position + Vector3.new(0, startHeight, 0), hrp.Position)
		
		_G.SiriusRejoinSkyPos = nil
		_G.SiriusRejoinCharPos = nil
	else
		-- Fresh execution - start from high up
		startHeight = 300
		midHeight = 150
		finalHeight = 10
		startBlur = 40
	...
	-- First descent - high to mid with flash
	local midPos = hrp.Position + Vector3.new(0, midHeight, 0)
	
	-- Flash in with fade
	tweenService:Create(flashFrame, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 0.75}):Play()
	task.wait(0.1)
	
	-- Flash out while zooming with longer fade
	...
	local camTween1 = tweenService:Create(
		...
		{CFrame = CFrame.new(midPos, hrp.Position)}
	)
	...
	-- Second descent - mid to character with flash
	local finalPos = hrp.Position + Vector3.new(0, finalHeight, 0)
	...
	local camTween2 = tweenService:Create(
		camera,
		TweenInfo.new(1.2, Enum.EasingStyle.Sine, Enum.EasingDirection.Out),
		{CFrame = CFrame.new(finalPos, hrp.Position)}
	)
